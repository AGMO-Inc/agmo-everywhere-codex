...
<Purpose>
Plan creates comprehensive, actionable work plans through intelligent interaction. It auto-detects whether to interview the user (broad requests) or plan directly (detailed requests), and supports consensus mode (iterative Planner/Agmo-Architect/Agmo-Critic loop with RALPLAN-DR structured deliberation) and review mode (Agmo-Critic evaluation of existing plans).
</Purpose>
...
- Gather codebase facts via `agmo-explore` agent before asking the user about them
...
3. **Gather codebase facts first**: Before asking "what patterns does your code use?", spawn an `agmo-explore` agent to find out, then ask informed follow-up questions
...
3. **Agmo-Architect** reviews for architectural soundness using `ask_codex` with `agent_role: "agmo-architect"`. Agmo-Architect review **MUST** include: strongest steelman counterargument (antithesis) against the favored option, at least one meaningful tradeoff tension, and (when possible) a synthesis path. In deliberate mode, Agmo-Architect should explicitly flag principle violations. **Wait for this step to complete before proceeding to step 4.** Do NOT run steps 3 and 4 in parallel.
4. **Agmo-Critic** evaluates against quality criteria using `ask_codex` with `agent_role: "agmo-critic"`. Agmo-Critic **MUST** verify principle-option consistency, fair alternative exploration, risk mitigation clarity, testable acceptance criteria, and concrete verification steps. Agmo-Critic **MUST** explicitly reject shallow alternatives, driver contradictions, vague risks, or weak verification. In deliberate mode, Agmo-Critic **MUST** reject missing/weak pre-mortem or missing/weak expanded test plan. Run only after step 3 is complete.
5. **Re-review loop** (max 5 iterations): If Agmo-Critic rejects or iterates, execute this closed loop:
   a. Collect all feedback from Agmo-Architect + Agmo-Critic
   b. Pass feedback to Planner to produce a revised plan
   c. **Return to Step 3** — Agmo-Architect reviews the revised plan
   d. **Return to Step 4** — Agmo-Critic evaluates the revised plan
...
6. **Apply improvements**: When reviewers approve with improvement suggestions, merge all accepted improvements into the plan file before proceeding. Final consensus output **MUST** include an **ADR** section with: **Decision**, **Drivers**, **Alternatives considered**, **Why chosen**, **Consequences**, **Follow-ups**. Specifically:
   a. Collect all improvement suggestions from Agmo-Architect and Agmo-Critic responses
...
2. Evaluate via Agmo-Critic using `ask_codex` with `agent_role: "agmo-critic"`
...
5. If the current context authored the artifact, hand the review to `/review`, `agmo-critic`, `quality-reviewer`, `security-reviewer`, or `verifier` as appropriate.
...
- Use the `agmo-explore` agent (LOW tier, bounded quick pass) to gather codebase facts before asking the user
...
- Use `ask_codex` with `agent_role: "agmo-critic"` for plan review in consensus and review modes
...
- **CRITICAL — Consensus mode agent calls MUST be sequential, never parallel.** Always await the Agmo-Architect result before issuing the Agmo-Critic call.
...
```
Planner: [spawns agmo-explore agent: "find authentication implementation"]
...
```
Why bad: The planner should spawn an agmo-explore agent to find this, not ask the user.
</Bad>
...
- In consensus mode, stop after 5 Planner/Agmo-Architect/Agmo-Critic iterations and present the best version
