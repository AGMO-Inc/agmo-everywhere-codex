...
Delegates to the `code-reviewer` and `agmo-architect` agents in parallel for a two-lane review:
...
2. **Launch Parallel Review Lanes**
   ...
   - **`agmo-architect` lane** - owns the devil's-advocate / design-tradeoff perspective
...
7. **Final Synthesis**
   - Combine the `code-reviewer` recommendation and the agmo-architect status into one final verdict
   - Deterministic merge gating rules:
     - If agmo-architect status is **BLOCK**, final recommendation is **REQUEST CHANGES**
     - Else if `code-reviewer` recommendation is **REQUEST CHANGES**, final recommendation is **REQUEST CHANGES**
     - Else if agmo-architect status is **WATCH**, final recommendation is **COMMENT**
     - Else final recommendation follows the `code-reviewer` lane
   - The final report must make agmo-architect blockers impossible to miss
...
The `agmo-architect` lane checks:
...
**APPROVE** - `code-reviewer` returns APPROVE and agmo-architect status is `CLEAR`
**REQUEST CHANGES** - `code-reviewer` returns REQUEST CHANGES or agmo-architect status is `BLOCK`
**COMMENT** - `code-reviewer` returns COMMENT with agmo-architect status `CLEAR`, agmo-architect status is `WATCH`, or only LOW/MEDIUM improvements remain
