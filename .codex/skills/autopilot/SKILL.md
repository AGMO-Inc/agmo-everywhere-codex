...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - If ambiguity remains high, run `agmo-explore` first for brownfield facts, then run `$deep-interview --quick <task>` before proceeding.
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   ...
   - Agmo-Architect (THOROUGH tier): Create technical specification
   - Output: `.omx/plans/autopilot-spec.md`

2. **Phase 1 - Planning**: Create an implementation plan from the spec
   - Agmo-Architect (THOROUGH tier): Create plan (direct mode, no interview)
   - Agmo-Critic (THOROUGH tier): Validate plan
...
3. **Phase 2 - Execution**: Implement the plan using Ralph + Ultrawork
   ...
   - THOROUGH-tier executor/agmo-architect roles: Complex tasks
...
5. **Phase 4 - Validation**: Multi-perspective review in parallel
   - Agmo-Architect: Functional completeness
...
- Use `ask_codex` with `agent_role: "agmo-architect"` for Phase 4 architecture validation
...
- `ralplan`: consensus planning (planner/agmo-architect/agmo-critic)
...
```
RALPLAN (consensus planning) -> team-exec (Codex CLI workers) -> ralph-verify (agmo-architect verification)
```
