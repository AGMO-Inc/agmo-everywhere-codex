...
**Cycle**: qa-tester → agmo-architect verification → fix → repeat
...
3. **ARCHITECT DIAGNOSIS**: Spawn agmo-architect to analyze failure
   ```
   delegate(role="agmo-architect", tier="THOROUGH", task="DIAGNOSE FAILURE:
   Goal: [goal type]
   Output: [test/build output]
   Provide root cause and specific fix recommendations.")
   ```

4. **FIX ISSUES**: Apply agmo-architect's recommendations
   ```
   delegate(role="executor", tier="STANDARD", task="FIX:
   Issue: [agmo-architect diagnosis]
