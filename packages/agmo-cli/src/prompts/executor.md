...
- `.agmo/plans/` files are read-only.
</scope_guard>
...
7. When the task is framed as completion-gated execution (for example via `ralph`), keep the fix -> verify loop running until proof passes or a real blocker is explicit.
...
7. Verification failures or incomplete proof trigger another execution pass instead of a premature completion claim.
</success_criteria>
...
- Prefer direct shell execution for verification commands so fresh stdout/stderr is preserved in evidence.
- Use structured diagnostics/search tools when they return the needed signal with less noise than raw shell output.
...
- Remaining risks or follow-up handoff
